...
CREATE OR ALTER PROCEDURE EtlOfflineOrder
AS
BEGIN
    BEGIN TRY
        ...
        PRINT 'Starting ETL process for Offline Orders...';

        -- Kiểm tra dữ liệu hiện có trong order_table và order_offline
        PRINT 'Current data in order_table:';
        SELECT COUNT(*) AS CurrentOrderCount FROM [dbo].[order_table];
        
        PRINT 'Current data in order_offline:';
        SELECT COUNT(*) AS CurrentOfflineOrderCount FROM [dbo].[order_offline];

        -- Tạo bảng tạm để lưu trữ dữ liệu từ OfflineDB.dbo.OrderTable mà chưa được ETL
        ...
        -- Chèn dữ liệu vào bảng tạm từ OfflineDB.dbo.OrderTable
        ...
        WHERE tgt.order_id IS NULL;  -- Chỉ lấy những đơn hàng chưa tồn tại trong order_offline

        -- In ra số lượng đơn hàng cần ETL
        PRINT 'New offline orders to be migrated:';
        SELECT COUNT(*) FROM #TempOrderTable;

        -- Nếu không có đơn hàng mới cần ETL thì kết thúc
        IF (SELECT COUNT(*) FROM #TempOrderTable) = 0
        BEGIN
            PRINT 'No new offline orders to migrate. ETL process completed.';
        ...
        PRINT 'Offline Order IDs to be migrated:';
		...
		CREATE TABLE #OrderIdMapping (
            OriginalId BIGINT,  -- ID từ OfflineDB.OrderTable
		...
		-- Bước 2: Chèn vào order_offline với ID gốc làm id và ID mới làm order_id
        ...
        SELECT 
            t.id,                  -- Sử dụng ID gốc làm ID của order_offline
        ...
        CREATE TABLE #TempOrderDetail (
            original_order_id BIGINT,  -- ID đơn hàng gốc từ OfflineDB
...
--        INSERT INTO [dbo].[order_table] (id, created, shipping_fee, status, subtotal, total)
--        SELECT 
--            id,
--            created, 
--            shipping_fee, 
...
--        -- Chèn vào order_offline và đặt branch_id = 2 cho tất cả
--        INSERT INTO [dbo].[order_offline] (order_id, staff_id)
--        SELECT 
--            id,            -- ID mới trong order_table
--            staff_id
--        FROM #TempOrderTable;
...
--        -- Chèn dữ liệu vào bảng tạm từ OfflineDB.dbo.OrderDetail cho các đơn hàng mới
--        INSERT INTO #TempOrderDetail (order_id, product_id, quantity, price)
--        SELECT 
--            d.order_id,
--            d.product_id,
--            d.quantity,
...
--        INNER JOIN #TempOrderTable t ON d.order_id = t.id;  -- Chỉ lấy chi tiết của các đơn hàng mới

--        -- Cập nhật branch_id cho các chi tiết đơn hàng
--        UPDATE td
--        SET branch_id = 2  -- Cập nhật branch_id = 2 cho tất cả chi tiết đơn hàng offline
--        FROM #TempOrderDetail td;

--        -- In ra số lượng chi tiết đơn hàng đã được chèn
--        PRINT 'Order details inserted:';
--        SELECT COUNT(*) FROM [dbo].[order_detail] 
--        WHERE order_id IN (SELECT id FROM #TempOrderTable);
